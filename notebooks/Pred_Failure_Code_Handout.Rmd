...
````
It looks like Process_temperature_K is correlated with Air_temperature_K and that Rotational_Speed_rpm is negatively correlated with Torque, so we will have to remove one of each respectively.
```{r}

pred_failure_lim <- pred_failure[c("Type", "Air_temperature_K", "Rotational_speed_rpm", "Tool.wear_min", "Failure_Type")]
str(pred_failure)
```
